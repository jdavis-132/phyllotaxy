...
fileList = ['angles_three_days_6cm.csv']

outData = pd.DataFrame(np.nan, index=[0], columns=['plant_name', 'JS_ID', 'plant_num', 'PI_num',
'img_date', 'voxel_len', 'h_0', 'h_1', 'h_2', 'h_3', 'h_4', 'h_5', 'h_6',
...
for i in range(0, len(fileList), 1):
    ...
    for line in currFile:
        ...
        if not re.search('Schnable', line[0]):
            continue
        # If the array has less than 8 columns it doesn't have useful data (i.e. <=1 height value)
        elif len(line) < 8:
            continue
        # If the accuracy is less than 0.70 or topology skeleton is false, ignore this line
        elif float(line[1]) < 0.7 or not line[2]:
            continue
        # If the first height value is error, ignore this line
        elif re.search('error', line[4]):
            continue
...
outData.to_csv(path + '/internodeData.csv', index=False)
